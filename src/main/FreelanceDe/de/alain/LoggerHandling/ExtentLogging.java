...
public class ExtentLogging {
	...
	public static void getResultWithScreenshotByFailTest(ITestResult getMethodName, ExtentTest logger,
			String ScreenshotPath, WebDriver driver, String dataProviderParameter) throws IOException {
		// Diese Methode ergibt das finale Ergebnis jedes TestSteps
		if (getMethodName.getStatus() == ITestResult.FAILURE) {

			// Der Screenshot wird ausgel�st, wo der Schritt im Testfall Fehlerhaft ist.
			logger.fail(getMethodName.getThrowable().getMessage(),
					MediaEntityBuilder.createScreenCaptureFromPath(ScreenshotPath).build());

			// Der Testfallname und der Suchbegriff werden im Report ausgeben.
			logger.log(Status.FAIL, MarkupHelper.createLabel(
					getMethodName.getName() + "_" + dataProviderParameter + " Test case FAILED due to below issues:",
					ExtentColor.RED));

			// Die Ausnahmen werden im Report ausgegeben.
